...
for PP = 1:nparticipants
    ...
    for i = 1:nTrials
        ...
        if isfile(k5file)
            ...
            varNames = string(k5.Properties.VariableNames);
    ...
    for i = 1:nTrials
        ...
        DAT(PP).Pnet = DAT(PP).pow_W(1:end) - DAT(PP).base;

        DAT(PP).CoT             = DAT(PP).Pnet / (DAT(PP).mass * speed);

        %labels
        label = strings(height(DAT(PP).config),1);
        for k = 1:height(DAT(PP).config)
        ...
        isforce = ~isnan(DAT(PP).config.Force_level_pct); %force pct check for value
        force_vals = DAT(PP).force_m(isforce);

        [force_sorted, idx_sort] = sort(force_vals); %force sort low -> high

        labels_force = label(isforce);
        labels_force_sorted = labels_force(idx_sort);

        cot_vals = DAT(PP).CoT(isforce);
        cot_sorted = cot_vals(idx_sort);
...
%% figure force in self-selection phase

% find self-selected trial indices per PP
...
%% plot distance to minimum S1 and S2
...
for PP = 1:nparticipants
    ...
    % Plot individuele dots and lines
...
all_y = [];
for PP = pp_select
    isForced = ~isnan(DAT(PP).config.Force_level_pct);
    comfort = DAT(PP).config.Comfort;
    force   = DAT(PP).force_m(:);
    n = min([length(comfort), length(force), height(DAT(PP).config)]);
    comfort = comfort(1:n);
...
%% Wilcoxon signed-rank tests: self-selected vs energetic minimum
...
% 3. |ΔF_S1| vs |ΔF_S2|  (paired, test for convergentie to minimum)
